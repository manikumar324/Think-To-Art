...
const Sidebar = ({isMenuOpen, setIsMenuOpen}) => {
  ...
  return (
    ...
    transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      {/* New Chat Button */}
      <button className='flex items-center justify-center w-full py-2 mt-10 text-white 
      ...
      <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
        ...
        chat.name.toLowerCase().includes(search.toLowerCase())).map((chat)=>(
          <div key={chat._id} onClick={()=> {navigate("/"); setSelectedChat(chat); setIsMenuOpen(false)}} className='p-2 px-4 dark:bg-[#57317C]/10 border
